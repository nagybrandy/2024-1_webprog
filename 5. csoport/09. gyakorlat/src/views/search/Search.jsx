...
import { getStudents } from "../../store/firebase/firebase";
...
export function Search() {
  ...
  return (
    <div className="join join-vertical w-full bg-base-300 shadow-xl overflow-x-hidden h-full pb-3 mt-5">
      ...
      <div className="pl-5">{isLoggedIn ? `User: ${user}` : "Loading..."}</div>
      <div className="pl-5">Number of tracks: {numberOfTracks} 
      <button className="btn btn-primary btn-sm ml-3" onClick={()=> dispatch(addTrack({title: "Új zene", artist: "Kozsó"}))}>AddTrack</button>
      <button className="btn btn-primary btn-sm ml-3" onClick={()=> dispatch(removeLastTrack())}>RemoveLastTrack</button>
      <button className="btn btn-primary btn-sm ml-3" onClick={()=> getStudents().then(e => console.log(e))}>getStudents</button>
      </div>
