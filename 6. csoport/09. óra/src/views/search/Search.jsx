...
import { addTrack, deleteLastTrack } from "../../store/actions/tracksSlice";
import { login } from "../../store/actions/userSlice";

export function Search() {
  const tracks = exampleTracks.slice(0,5);
  const {tracks: tracksList, numberOfTracks} = useSelector(state => state.tracks)
  const dispatch = useDispatch()
  console.log(tracksList, numberOfTracks)
  ...
  return (
    <div className="join join-vertical w-full bg-base-300 shadow-xl overflow-x-hidden h-full pb-3 mt-5">
      ...
      <div className="p-5">Number of tracks: {numberOfTracks} 
        <div className="btn btn-primary btn-sm" onClick={() => dispatch(addTrack({title: "Új zene", artist: "Hellomizu"}))}>AddTrack</div>
        <div className="btn btn-primary btn-sm" onClick={() => dispatch(deleteLastTrack())}>RemoveLastTrack</div>
      </div>
