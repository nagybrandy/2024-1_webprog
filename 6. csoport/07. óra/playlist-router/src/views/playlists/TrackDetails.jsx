...
export function TrackDetails() {
  ...
  const selectedTrack = params.trid ? exampleTracks.find((t) => t.id === parseInt(params.trid)) : null;
  return (
        ...
        <div className="md:flex md:flex-wrap p-5 ">
          <div className="md:w-8/12 w-full mx-auto">
            ...
            <div className="rating mt-2 mb-5">
              {Array.from({length: selectedTrack.rating}).map((_,i) => <input type="radio" name="rating-4" className="mask mask-star-2 bg-accent" key={i}/>)}
              {Array.from({length: 5-selectedTrack.rating}).map((_,i) => <input type="radio" name="rating-4" className="mask mask-star-2 bg-neutral" key={i}/>)}
            </div>
