    ...
    <div className="join join-vertical w-full bg-base-300 shadow-xl overflow-x-hidden h-full">
      ...
      <div className="overflow-y-scroll w-full join join-vertical pl-[0.6rem]  overflow-hidden">
        {selectedPlaylist.tracks.map((track, i) => (
            <Link to={`/playlists/${params.plid}/track/${i+1}`} key={track.id} className={`btn rounded-lg items-start p-3 border-0 hover:bg-base-200 ${selectedTrackId === track.id ? "bg-base-200" : "bg-base-300"}`} onClick={()=> setSelectedTrackId(track.id)}>
